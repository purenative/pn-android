...
abstract class IBillingService {
    ...
    fun subscriptionEnded() {
        findUiHandler().post {
            subscriptionEndedInternal()
        }
    }

    private fun subscriptionEndedInternal() {
        for (subscriptionServiceListener in subscriptionServiceListeners) {
            subscriptionServiceListener.onSubscriptionEnded()
        }
    }

    fun subscriptionAcknowledged() {
        findUiHandler().post {
            subscriptionAcknowledgedInternal()
        }
    }

    private fun subscriptionAcknowledgedInternal() {
        for (subscriptionServiceListener in subscriptionServiceListeners) {
            subscriptionServiceListener.onSubscriptionAcknowledged()
        }
    }
    ...
    fun onBillingClientError(error: String, fatal: Boolean) {
        findUiHandler().post {
            for (billingServiceListener in billingClientConnectedListeners) {
                billingServiceListener.onError(error, fatal)
            }
        }
    }
